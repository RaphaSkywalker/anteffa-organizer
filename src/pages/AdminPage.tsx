...
import { UserPlus, Users, Shield, Cake, Calendar, Trash2, Loader2, Search, PlusCircle, LayoutGrid, Edit3, XCircle, RefreshCw, CheckCircle2, ChevronDown } from "lucide-react";
...
type Tab = "employees" | "teams" | "birthdays" | "dates";
...
export default function AdminPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [emps, tms, bdays, cdates] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else endpoint = editingItem ? `/api/admin/company-dates/${editingItem.id}` : "/api/admin/company-dates";
    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-wrap gap-2 p-1.5 bg-card/50 backdrop-blur-md border border-border rounded-2xl w-fit shadow-sm">
        {[
          ...
          <button
            key={tab.id}
            onClick={() => { setActiveTab(tab.id as Tab); setEditingItem(null); }}
            ...
            <div className="flex items-center justify-between mb-8 relative">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-foreground">
                  {editingItem ? "Editar" : "Cadastrar"} {activeTab === "employees" ? "Funcionário" : activeTab === "teams" ? "Equipe" : activeTab === "birthdays" ? "Aniversariante" : "Data"}
                </h2>
              </div>
              {editingItem && (
        ...
        <motion.div key={activeTab + "-list"} {...fadeIn} className="lg:col-span-8 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <h2 className="text-xl font-bold text-foreground flex items-center gap-3">
              {activeTab === "employees" ? <Users className="w-5 h-5 text-primary" /> : activeTab === "teams" ? <LayoutGrid className="w-5 h-5 text-primary" /> : activeTab === "birthdays" ? <Cake className="w-5 h-5 text-primary" /> : <Calendar className="w-5 h-5 text-primary" />}
              Lista de {activeTab === "employees" ? "Funcionários" : activeTab === "teams" ? "Equipes" : activeTab === "birthdays" ? "Aniversariantes" : "Datas"}
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {loading ? (
              Array(4).fill(0).map((_, i) => <div key={i} className="h-28 rounded-3xl bg-card/30 animate-pulse border border-border/50" />)
            ) : (() => {
              const list = activeTab === "employees" ? employees : activeTab === "teams" ? teams : activeTab === "birthdays" ? birthdays : companyDates;
